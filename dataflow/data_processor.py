...
def process_file(
    ...
    if replace:
        ...
        if deleted:
            logger.info(f"    Replaced existing '{dataset_name}' ({deleted} record(s) removed)")

    rules = _load_rules(rules_file_path)
    if rules:
        logger.info(f"    Rules loaded: {list(rules.keys())}")
    else:
    ...
    logger.info(f"    Read {total_rows} rows, columns: {columns}")
    ...
    if rules and valid_records:
        ...
        if model:
            ...
            logger.info(f"    {len(valid_records)} records in dynamic table '{schema_obj.table_name}'")
    ...
    logger.info(f"    Done: {len(valid_records)} valid, {total_rows - len(valid_records)} invalid")
...
def process_generic_dataset(
    ...
    logger.info(f"    Saved {len(valid_records)} valid records out of {len(raw_records)}")
    return dataset_obj, valid_records
...
if __name__ == "__main__":
    ...
    logger.info(f"Total DataRecords: {DataRecord.objects.count()}")
    logger.info(f"Total CleaningLogs: {CleaningLog.objects.count()}")
